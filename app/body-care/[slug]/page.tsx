...
export async function generateStaticParams() {
  return getAllReviewsIncludingHidden("body-care").map((r) => ({ slug: r.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const review = getReview("body-care", slug);
  if (!review) return {};
  const description = review.summary || `${review.brand} ${review.name} review.`;
  return {
    title: `${review.name} by ${review.brand}`,
    description,
    alternates: { canonical: `/body-care/${review.slug}` },
...
export default async function BodyCareReviewPage({ params }: Props) {
  const { slug } = await params;
  const review = getReview("body-care", slug);
  if (!review) notFound();
  const relatedPrimers = getPrimersForProduct(review.slug);
  const { prev, next } = getAdjacentReviews("body-care", review.slug);
  const related = getRelatedReviews(review);

  return (
    <article>
      <ReadingProgress />
      <ContinueReading path={`/body-care/`} />
      ...
      <Container className="py-10">
        <Breadcrumb
          trail={[
            ...
            { name: review.name, href: `/body-care/${review.slug}` },
          ]}
        />
        <div className="flex items-center justify-between gap-4">
          ...
          <CopyLink path={`/body-care/${review.slug}`} />
        </div>
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_320px]">
          <div className="space-y-10">
            ...
            <ReaderNote kind="body-care" slug={review.slug} />
          </div>
          ...
          next={
            next
              ? {
                  ...
                  href: `/body-care/${next.slug}`,
                }
              : null
          }
          label="Body-care pagination"
        />
      </Container>
    </article>
  );

...
export async function generateStaticParams() {
  return getAllReviewsIncludingHidden("hair-care").map((r) => ({ slug: r.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const review = getReview("hair-care", slug);
  if (!review) return {};
  const description = review.summary || `${review.brand} ${review.name} review.`;
  return {
    title: `${review.name} by ${review.brand}`,
    description,
    alternates: { canonical: `/hair-care/${review.slug}` },
...
export default async function HairCareReviewPage({ params }: Props) {
  const { slug } = await params;
  const review = getReview("hair-care", slug);
  if (!review) notFound();
  const relatedPrimers = getPrimersForProduct(review.slug);
  const { prev, next } = getAdjacentReviews("hair-care", review.slug);
  const related = getRelatedReviews(review);

  return (
    <article>
      <ReadingProgress />
      <ContinueReading path={`/hair-care/`} />
      ...
      <Container className="py-10">
        <Breadcrumb
          trail={[
            ...
            { name: review.name, href: `/hair-care/${review.slug}` },
          ]}
        />
        <div className="flex items-center justify-between gap-4">
          ...
          <CopyLink path={`/hair-care/${review.slug}`} />
        </div>
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_320px]">
          <div className="space-y-10">
            ...
            <ReaderNote kind="hair-care" slug={review.slug} />
          </div>
          ...
          next={
            next
              ? {
                  ...
                  href: `/hair-care/${next.slug}`,
                }
              : null
          }
          label="Hair-care pagination"
        />
      </Container>
    </article>
  );
